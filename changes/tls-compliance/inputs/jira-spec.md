...
**Primary ticket:** [OCPSTRAT-2611 — Central TLS Consistency](https://redhat.atlassian.net/browse/OCPSTRAT-2611)

**Related tickets:** OCPSTRAT-3145 (Hybrid ML-KEM), OCPSTRAT-3123 (TLS Group Preferences), OCPSTRAT-2361

**Source:** `ADR-TLS-Compliance.md` (Jira MCP unavailable; ADR used as spec input)
...
- `require_pq_kem` is an **application-level override** controlled by `ZeroTrustWorkloadIdentityManager.spec.requirePQKEM`. When true, the operator injects `experimental.require_pq_kem = true` into SPIRE Server, SPIRE Agent, and SPIRE Controller Manager configs. The central TLS profile's `minTLSVersion`, `ciphers`, and `groups` are **not injected** into SPIRE config. SPIRE's own `tlspolicy` module handles enforcement internally.
...
| requirePQKEM (ZTWIM CRD) | TLS adherence                                                | TLS security profile                 | Final TLS used                                                                                                   | PQC behavior                                                                          |
| ------------------------ | ------------------------------------------------------------ | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
...
| **Inject requirePQKEM into OIDC Discovery Provider config**    | support/oidc-discovery-provider/main.go                                                                            | In newListenerWithServingCert() and newACMEListener() after creating tlsConfig object.                                                                                                                                                                                                     |
...
| **Warning event on PQ + strict adherence coexistence**         | pkg/controller/spire-server/controller.go, pkg/controller/spire-agent/controller.go                                | When `spec.requirePQKEM=true` and `tlsAdherencePolicy=StrictAllComponents`, emit Warning event that application-level PQ policy overrides central profile injection.                                                                                                                       |
