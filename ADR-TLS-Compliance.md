...
(b) Hybrid ML-KEM support (OCPSTRAT-3145) — SPIRE workload identity workflows (server–agent communication, workload–workload mTLS) can enforce quantum-safe hybrid key exchange (X25519MLKEM768) via the upstream `experimental.require_pq_kem` config, exposed through the ZTWIM CRD.

**2. Precedence: application-specific `require_pq_kem` takes priority over central TLS profile.**  
When `requirePQKEM: true` is set in the ZTWIM CRD, SPIRE's own PQ policy takes full control (`CurvePreferences: [X25519MLKEM768]`, TLS 1.3 minimum). The central TLS profile settings (minTLSVersion, ciphers, groups) are NOT injected. When `requirePQKEM` is absent or false, the central TLS profile is enforced (if adherence is StrictAllComponents) or Go defaults (component defaults) apply — Go 1.24+ opportunistically prefers X25519MLKEM768 but gracefully falls back to classical curves if the peer does not support it.

**3. Change propagation: APIServer object changes cause rolling restart of operator, then operands.**  
Per the established guidelines: when the cluster admin changes the TLS profile or `tlsAdherencePolicy` on the APIServer resource, the ZTWIM operator's SecurityProfileWatcher fires, cancels its context, and the operator pod restarts with the new profile. During reconciliation, the operator resolves the updated profile into SPIRE config, updates ConfigMaps, and the resulting hash change triggers rolling restarts of SPIRE operand workloads (StatefulSet, DaemonSet, Deployments).
...
- `requirePQKEM` (application-level) takes precedence over the central TLS profile when set. When `requirePQKEM` is absent: central TLS profile enforced if strict; otherwise Go defaults apply (opportunistic PQC with classical fallback).  
...
```mermaid
flowchart TD
    Start["TLS Config Resolution"] --> CheckPQ{"require_pq_kem\nset in CR?"}
    ...
    CheckPQ -->|No| CheckAdherence{"tlsAdherence ==\nStrictAllComponents?"}
...
- `require_pq_kem` is an **application-level override** that maps directly to SPIRE's `experimental.require_pq_kem`.  When set, the central TLS profile's `minTLSVersion`, `ciphers`, and `groups` are **not injected** into SPIRE config. SPIRE's own `tlspolicy` module handles enforcement internally.
- When `require_pq_kem` is absent or false, the existing Layer 2 injection applies: resolve central profile → inject `minTLSVersion`, `cipherSuites`, and (future) `groups` into SPIRE config. If the central profile is also not enforced, Go defaults provide opportunistic PQC — Go 1.24+ prefers X25519MLKEM768 when both peers support it, and gracefully falls back to classical curves otherwise.

### **Determining Effective TLS Configuration**


| requirepqkem (CRD) | TLS adherence                                                | TLS security profile                 | Final TLS used                                                                                                   | PQC behavior                                                                          |
| ------------------ | ------------------------------------------------------------ | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
...
| **Add PQC config to CRD**                                      | api/v1alpha1/spireserverconfigtypes.go, spireagentconfigtypes.go                                                   | Add `RequirePQKEM *bool` field to SpireServerSpec and SpireAgentSpec. `true` maps to `experimental.require_pq_kem = true` in SPIRE config (hybrid X25519MLKEM768 only, rejects non-PQ clients). `false`/absent defers to central TLS profile or Go defaults. |
| **Inject requirepqkem into SPIRE Server config**               | pkg/controller/spire-server/configmap.go                                                                           | In `generateServerConfMap`: if `RequirePQKEM` is true, emit `"experimental": {"require_pq_kem": true}` and skip injecting minTLSVersion/cipherSuites from the central profile.                                                                               |
| **Inject requirepqkem into SPIRE Agent config**                | pkg/controller/spire-agent/configmap.go                                                                            | In `generateAgentConfig`: if `RequirePQKEM` is true, emit `"experimental": {"require_pq_kem": true}` in the agent config.                                                                                                                                    |
...
- Phase 3: CRD `requirePQKEM` field + `experimental.require_pq_kem` injection into SPIRE Server and Agent config.  
